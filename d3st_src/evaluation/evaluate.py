...
import numpy as np
import logging
from d3st_src.evaluation import metrics
...
def main(args):
    in_domain_services = get_in_domain_services(
        os.path.join(args.dstc8_data_dir, args.eval_set, "schema.json"),
        os.path.join(args.dstc8_data_dir, "train", "schema.json"))
    with open(os.path.join(args.dstc8_data_dir, args.eval_set, "schema.json")) as f:
        eval_services = {}
    ...
    logging.info("len(dataset_hyp)=%d, len(dataset_ref)=%d", len(dataset_hyp), len(dataset_ref))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prediction_dir", help="Directory in which all JSON files combined are predictions of the"
                                                 " evaluation set on a single model checkpoint. We evaluate these JSON"
                                                 " files by DSTC8 metrics.")
    parser.add_argument("--dstc8_data_dir", help="Directory for the downloaded DSTC8 data, which contains the dialogue "
                                                 "files and schema files of all datasets (train, dev, test)")
    parser.add_argument("--eval_set", choices=["train", "dev", "test"], help="Dataset split for evaluation.")
    parser.add_argument("--output_metric_file", help="Single JSON output file containing aggregated evaluation metrics "
                                                     "results for all predictions files in FLAGS.prediction_dir.")
    parser.add_argument("--joint_acc_across_turn", action="store_true", help="Whether to compute joint accuracy across "
                                                                             "turn instead of across service. "
                                                                             "Should be set to True when conducting "
                                                                             "multiwoz style evaluation.")
    parser.add_argument("--use_fuzzy_match", action="store_true", help="Whether to use fuzzy string matching when "
                                                                       "comparing non-categorical slot values. Should "
                                                                       "be set to False when conducting multiwoz style "
                                                                       "evaluation.")
