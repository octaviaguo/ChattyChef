...
class DataModule(LightningDataModule):
    def __init__(
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(self.tokenizer_path, use_fast=True, model_max_length=1024)
    ...
    def setup(self, stage: str = None):
        ...
        if stage == 'predict':
            ...
            for i in range(len(splits)):
                column_names = ['history', 'user_utt', 'intent']
                if 'file' in raw_datasets[i].column_names:
                    column_names.append('file')
                if 'utt_id' in raw_datasets[i].column_names:
                    column_names.append('utt_id')
                ...
                lm_dataset.set_format(type="torch", columns=columns)
                self.dataset[splits[i]] = lm_dataset
    ...
    def dataset_mapping_function(self, examples):
        ...
        for sample_idx, history, user_utt, intent in zip(sample_ids, histories, user_utts, intents):
            ...
            gold_intent_prompt = []
            for _intent in intent.split(' '):
                gold_intent = _intent[:-1]
                gold_intent_idx = self.intent2idx[gold_intent]
                gold_intent_prompt.append(f'{intent_id_remap[gold_intent_idx]}')

            src_text = f'{description_prompt} {conversation}'
            tgt_text = '[intents] ' + ' '.join(gold_intent_prompt)

            tokenized_src_text = self.tokenizer(src_text)
            tokenized_tgt_text = self.tokenizer(tgt_text)
    ...
    def decode_predictions(self, predictions):
        ...
        for batch in predictions:
            ...
            for i in range(len(batch_results)):
                results.append([sample_idx[i].item(), batch_input_ids[i] + ' ' + batch_results[i]])
