...
from dst.constant.newdataset_constant import *
...
class DataModule(LightningDataModule):
    ...
    def setup(self, stage: str = None):
        ...
        column_names = ['Context', 'Response', 'Knowledge', 'Index', 'File', "Current_step_idx", "Next_step_idx",
                        "intents"]
    ...
    def dataset_mapping_function(self, examples):
        ...
        current_step_ids = examples['Current_step_idx']
        next_step_ids = examples['Next_step_idx']
        intents = examples['intents']

        all_inputs = []
        all_lm_labels = []
        for context, kb, response, current_step_idx, next_step_idx, intent in\
                zip(contextes, kbs, responses, current_step_ids, next_step_ids, intents):
            context = context.replace(' <|EOS|> ', ' ')
            history_ids = self.tokenizer.encode(context + ' <|Knowledge|> ', add_special_tokens=False)

            recipe_steps = kb.split(" <|EOS|> ")
            recipe_step_titles = []
            recipe_step_with_step_idx = []
            for i, recipe_step in enumerate(recipe_steps):
                tmp = recipe_step.split(" <|step|> ")
                # recipe_step_titles.append("Step " + str(i + 1) + ": " + tmp[0])
                recipe_step_titles.append(" - " + tmp[0])
                recipe_step_with_step_idx.append(recipe_step_titles[-1] + " " + tmp[1])

            if self.align_mode == 0:
                grounded_kb = ' '.join(recipe_step_with_step_idx)
                grounded_kb_ids = self.tokenizer.encode(grounded_kb, add_special_tokens=False)
                gen_token = self.tokenizer.encode(" => [system] " + self.tokenizer.eos_token, add_special_tokens=False)
            # cutoff
            elif self.align_mode == 1:
                grounded_kb = ' '.join(recipe_step_with_step_idx[current_step_idx:])
                grounded_kb_ids = self.tokenizer.encode(grounded_kb, add_special_tokens=False)
                gen_token = self.tokenizer.encode(" => [system] " + self.tokenizer.eos_token, add_special_tokens=False)
            # center cutoff
            else:
                tmp1 = max(0, current_step_idx - 1)
                tmp2 = min(len(recipe_step_with_step_idx), current_step_idx + 2)
                grounded_kb = ' '.join(recipe_step_with_step_idx[tmp1:tmp2])
                grounded_kb_ids = self.tokenizer.encode(grounded_kb, add_special_tokens=False)
                gen_token = self.tokenizer.encode(" => [system] " + self.tokenizer.eos_token, add_special_tokens=False)

            if self.use_intent:
                intent_desc = [INTENT_DESCRIPTIONS[_] for _ in intent.split("; ")]
    ...
    def dataset_mapping_for_generation_function(self, examples):
        ...
        all_prompt_lengths = []
        all_sample_ids = []
        for idx, context, kb, response, current_step_idx, next_step_idx, intent in \
                zip(sample_ids, contextes, kbs, responses, current_step_ids, next_step_ids, intents):
            context = context.replace(' <|EOS|> ', ' ')
            history_ids = self.tokenizer.encode(context + ' <|Knowledge|> ', add_special_tokens=False)
    ...
    def decode_predictions(self, predictions):
        ...
        for batch in predictions:
            ...
            for i, result in enumerate(batch_results):
                ...
                # results.append([sample_idx[i].item(), result.split(" => ")[-1]])
