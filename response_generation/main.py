...
from data_modules.combined_data_module import DataModule
# from data_modules.cookdial_data_module import DataModule
from model import DialogGeneration
...
def main(args):
    ...
    if args.do_train:
        ...
        for batch in dm.train_dataloader():
            ...
            print(dm.tokenizer.batch_decode(
                torch.where(batch['labels'][:3] != -100, batch['labels'][:3], dm.tokenizer.eos_token_id), skip_special_tokens=False))
            break
        ...
        # lt_model = DialogGeneration(**dict_args)

        if args.finetune_from_checkpoint:
            lt_model = DialogGeneration.load_from_checkpoint(args.finetune_from_checkpoint,
    ...
    if args.do_test or args.do_validation:
        if args.do_validation:
             dict_args['val_collate'] = 1
        dm = DataModule(**dict_args)
        if args.do_test:
            dm.setup("test")
            ...
            dm.setup("validation")
            predict_dataloader = dm.val_dataloader()
        for batch in predict_dataloader:
            print(dm.tokenizer.batch_decode(batch['input_ids'][:3], skip_special_tokens=False))
            break
 
        lt_model = DialogGeneration.load_from_checkpoint(args.best_checkpoint,
        ...
        gen_kwargs = {'num_beams': 5,
                      'max_length': args.max_source_length + args.max_target_length, # args.max_recipe_length + args.max_history_length + args.max_target_length,
...
if __name__ == '__main__':
    ...
    parser.add_argument("--max_target_length", default=64, type=int,
                        help="The maximum total input sequence length after tokenization. Sequences longer "
                             "than this will be truncated, sequences shorter will be padded.")
    parser.add_argument("--max_recipe_length", default=386, type=int)
    parser.add_argument("--max_history_length", default=386, type=int)
    ...
    parser.add_argument("--align_mode", type=int, default=0)
    parser.add_argument("--use_intent", action="store_true", help="whether to use intent information")
